...
def write_database(reserved_hostnames):
    ...
    for hostnames in reserved_hostnames:
        ...
        if hostname not in existing_hosts:
            query = "INSERT INTO devices (ipaddr, hostname, lastseen) VALUES (%s, %s, %s)"
            values = (ipaddress, hostname, lastseen)
            cursor.execute(query, values)
            print(f"Se insertó un nuevo registro para el hostname {hostname}.")
